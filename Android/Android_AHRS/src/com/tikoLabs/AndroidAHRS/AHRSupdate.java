...
public class AHRSupdate {
    ...
    static volatile float integralFBx = 0.0f; // integral error terms scaled by Ki
    ...
    static void MahonyAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
        ...
        if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
            ...
            if(twoKi > 0.0f) {
                integralFBx += twoKi * halfex * timeDelta;    // integral error scaled by Ki
                integralFBy += twoKi * halfey * timeDelta;
                integralFBz += twoKi * halfez * timeDelta;
        ...
        gx *= (0.5f * timeDelta);     // pre-multiply common factors
        gy *= (0.5f * timeDelta);
        gz *= (0.5f * timeDelta);
        qa = q0;
        qb = q1;
